...
try {
  // Hide require from Turbopack so missing optional deps don't produce build warnings
...
const ENABLE_CONVERT = process.env.ENABLE_FFMPEG_CONVERT === "1";
...
try {
  ...
  const msg = e?.message || "No working ffmpeg binary found.";
  return new Response(msg, { status: 501, headers: { "Content-Type": "text/plain; charset=utf-8" } });
}
