...
type ColorMotionMeta = {
  ...
  // keep label for compatibility if needed elsewhere
  label: string;
...
/* =========================
   Motion (Emotion) map (Color path)
========================= */

const COLOR_MOTIONS: ColorMotionMeta[] = [
  {
    id: "magic",
...
const COLOR_MOTIONS_BY_ID: Record<EmotionId, ColorMotionMeta> = COLOR_MOTIONS.reduce(
  (acc, e) => {
    acc[e.id] = e;
    return acc;
  },
  {} as Record<EmotionId, ColorMotionMeta>
);
// Back-compat aliases (page UI still references COLOR_EMOTIONS / COLOR_EMOTIONS_BY_ID)
const COLOR_EMOTIONS = COLOR_MOTIONS;
const COLOR_EMOTIONS_BY_ID = COLOR_MOTIONS_BY_ID;

/* =========================
   Per-chord motion text (Color) — use the updated "two-paths" copy
========================= */
        ...
        {COLOR_EMOTIONS.map((e) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontWeight: 800 }}>{e.motion}</span>
<span style={{ opacity: 0.7 }}>({e.emotion})</span>
            </button>
          );
        })}
      </div>
...
async function exportEmotionClip(emotion: ColorEmotionMeta) {
  ...
  const HIT_SEC = 1.0;     // 1000ms per hit
const REPEATS = 2;       // each chord 2 times
const stepsTotal = chords.length * REPEATS; // 8 steps
const totalSec = stepsTotal * HIT_SEC;      // exactly 8.0s
...
if (!ctxMaybe) throw new Error("no 2D ctx");
const ctx = ctxMaybe; // non-null alias for TypeScript
  ...
  const chordIdx = Math.floor(step / REPEATS); // 0,0,1,1,2,2,3,3...
  const chordSymbol = chords[chordIdx] ?? "";
  const rh = triadNamesInRH(chordSymbol);
  const lh = rootNameInLH(chordSymbol);
  const notes = [lh, ...rh].filter(Boolean);

  const at = t0 + step * HIT_SEC;
  ...
  const drawFrame = () => {
    ...
    const step = Math.min(stepsTotal - 1, Math.floor(elapsed / HIT_SEC));
const chordIdx = Math.min(chords.length - 1, Math.floor(step / REPEATS));
const chordSymbol = chords[chordIdx] ?? null;
    ...
    ctx.fillText(
  `${emotion.emoji} ${emotion.motion} (${emotion.emotion})`,
  (FRAME_W * SCALE) / 2,
  FRAME_H * SCALE * 0.13 + Y_OFFSET
);
    ctx.restore();

    // Per-chord description (single line at a time) with One/Two/Three/Four label
const lines = COLOR_CHORD_TEXT[emotion.id] ?? ["", "", "", ""];
const labels = ["One", "Two", "Three", "Four"] as const;

const phrase = lines[chordIdx] ?? "";
const label = labels[chordIdx] ?? "One";

const KEYBOARD_Y = FRAME_H * SCALE * 0.50 + Y_OFFSET;

// Layout box
const boxW = FRAME_W * SCALE * 0.88;
const boxLeft = (FRAME_W * SCALE - boxW) / 2;

// Prepare text metrics first (so we can position boxTop using wrapped.length)
ctx.save();
ctx.textAlign = "left";
ctx.textBaseline = "top";
ctx.fillStyle = "#111827";

// --- label + description layout (no overlap) ---
const LABEL_SIZE = 42 * SCALE;   // smaller than title
const GAP = 10 * SCALE;          // spacing between label and sentence

// 1) Measure label width using LABEL font (critical)
ctx.font = `800 ${LABEL_SIZE}px system-ui, -apple-system, sans-serif`;
const labelText = `${label}: `;
const labelW = ctx.measureText(labelText).width;

// 2) Fit description into 2 lines using DESC font and the remaining width
const descMin = 32 * SCALE;
let descSize = 48 * SCALE; // starting size for description text (already working)
const descMaxWidth = boxW - (labelW + GAP);

function wrapIntoTwoLines(text: string, maxWidth: number): [string, string] {
...
let wrapped = ["", ""] as [string, string];

while (descSize >= descMin) {
  ctx.font = `700 ${descSize}px system-ui, -apple-system, sans-serif`;
  wrapped = wrapIntoTwoLines(phrase, descMaxWidth) as [string, string];

  const ok1 = ctx.measureText(wrapped[0]).width <= descMaxWidth + 0.5;
  const ok2 = ctx.measureText(wrapped[1]).width <= descMaxWidth + 0.5;

  if (ok1 && ok2) break;
  descSize -= 2 * SCALE;
}

const lineH = Math.round(descSize * 1.18);
const linesCount = wrapped[1] ? 2 : 1;
const boxTop = KEYBOARD_Y - linesCount * lineH - 12 * SCALE;

// 3) Draw label (smaller)
ctx.font = `800 ${LABEL_SIZE}px system-ui, -apple-system, sans-serif`;
ctx.fillStyle = "rgba(17,24,39,0.75)";
ctx.fillText(labelText, boxLeft, boxTop);

// 4) Draw description (starts after labelW + GAP)
ctx.font = `700 ${descSize}px system-ui, -apple-system, sans-serif`;
const textX = boxLeft + labelW + GAP;

if (wrapped[0]) ctx.fillText(wrapped[0], textX, boxTop);
if (wrapped[1]) ctx.fillText(wrapped[1], textX, boxTop + lineH);

ctx.restore();

// Chord caption (keep under the keyboard)
ctx.save();
ctx.textAlign = "center";
ctx.textBaseline = "top";
...
const chordCaptionY = KEYBOARD_Y + 330 * SCALE;
const chordCaption = chordSymbol ? chordSymbol.replace(/ /g, "") : "";
const boxCenterX = boxLeft + boxW / 2;

ctx.fillText(chordCaption, boxCenterX, chordCaptionY);
ctx.restore();
    ...
    >
      <div style={{ maxWidth: 560, margin: "0 auto", padding: 12, boxSizing: "border-box" }}>
        ...
        <div style={{ width: "100%", maxWidth: 180, margin: "0 auto", aspectRatio: "1 / 1" }}>
          <button
            type="button"
            aria-label={`Color circle for ${selectedMeta.motion}`}
            style={{
