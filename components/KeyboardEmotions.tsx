...
export default function KeyboardEmotions({
  ...
  const highlightedSecondary = useMemo(() => new Set(secondaryNotes), [secondaryNotes]);
...
const fill = isPrimary
  ...
  ? keyColorSecondary
  : "#000000";
              const shownNote = highlightedPrimary.has(k.noteFlat) ? k.noteFlat : k.noteSharp;
const baseLabel = displayMap?.[shownNote] ?? stripOct(shownNote);
const labelText = prettyBase(baseLabel);
